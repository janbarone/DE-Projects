{{ config(
    materialized='table',
    post_hook=[
        "create index if not exists {{ this.schema }}_fact_mpm_match_idx on {{ this }}(match_id)",
        "create index if not exists {{ this.schema }}_fact_mpm_player_idx on {{ this }}(player_slot)",
        "create index if not exists {{ this.schema }}_fact_mpm_minute_idx on {{ this }}(minute)"
    ]
) }}
...
order by s.match_id, s.player_slot, s.minute
