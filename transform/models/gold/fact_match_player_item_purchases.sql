{{ config(
    materialized='table',
    post_hook=[
        "create index if not exists {{ this.schema }}_fact_mpip_match_idx on {{ this }}(match_id)",
        "create index if not exists {{ this.schema }}_fact_mpip_player_idx on {{ this }}(player_slot)",
        "create index if not exists {{ this.schema }}_fact_mpip_minute_idx on {{ this }}(minute)"
    ]
) }}
