...
select
    ...
    (p.value->'gold_t'->((s.ord - 1)::int))::int   as gold,
    (p.value->'xp_t'->((s.ord - 1)::int))::int     as xp,
    (p.value->'lh_t'->((s.ord - 1)::int))::int     as last_hits,
    (p.value->'dn_t'->((s.ord - 1)::int))::int     as denies
from new_matches m
