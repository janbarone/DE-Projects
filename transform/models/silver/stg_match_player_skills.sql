{{ config(
    ...
    post_hook=[
        "create index if not exists {{ this.schema }}_stg_mps_match_idx on {{ this }}(match_id)",
        "create index if not exists {{ this.schema }}_stg_mps_player_idx on {{ this }}(player_slot)"
    ]
) }}
